...
class TermsAgreementAdapter(
    ...
    override fun upsertTermsAgreement(
        user: User,
        termsAgreementRequests: List<TermsAgreementRequest>,
    ) {
        val termsAgreementEntities =
            toTermsAgreementEntities(
                user = user,
    ...
    private fun toTermsAgreementEntities(
        user: User,
        request: List<TermsAgreementRequest>,
    ): List<TermsAgreementEntity> {
        val userTermsAgreementsMap =
            termsAgreementEntityRepository
                .findAllByUserId(user.userId)
